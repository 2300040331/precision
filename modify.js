...
const cssPath = 'services-premium.css';
let css = fs.readFileSync(cssPath, 'utf8');
if (!css.includes('NEW INTRO SECTION')) {
...
`;
    ...
    const regex = /<!-- 2 SERVICE OVERVIEW -->[\s\S]*?<\/section>/;
    if (regex.test(content)) {
        content = content.replace(regex, introHtml);
        fs.writeFileSync(filename, content, 'utf8');
        console.log(`Updated ${filename}`);
    } else {
        console.log(`Could not find SERVICE OVERVIEW in ${filename}`);
    }
}
