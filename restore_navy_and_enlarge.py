...
# Restore the dark navy boxes design exactly as it was
navy_boxes_css = """/* ── Premium Golden Box Cards ── */
.founder-card {
    cursor: pointer;
    position: relative;
    display: flex;
...
pattern = r'/\* ── Premium Golden Image Boxes ── \*/.*?/\* ── Modals ── \*/'
css = re.sub(pattern, navy_boxes_css + '\n\n/* ── Modals ── */', css, flags=re.DOTALL)

# Increase max-width of the grid container to make boxes bigger
css = css.replace('max-width: 1400px;', 'max-width: 1600px;')

with open(css_path, 'w') as f:
    f.write(css)

print("Restored navy boxes and increased grid max-width/font sizes.")
